...
3. **Primary response variables** — physics `averageFPS`, `statsSamplesAverage.BODY_COUNT`, `STEP_MS`.
...
5. **Replication** — `pnpm bench:headed:median` (or `run-headed-median.mjs`): use **≥5 runs**, report **median** and **CV** (coefficient of variation). Lower CV on body counts usually means more comparable load. Prints **spatial** `NEIGHBOR_MS` / `GRID_CELLS_CHECKED` medians when present. Optional JSON: `pnpm bench:headed:spatial-confirm` (writes `tests/results/research-spatial-headed.json`).
