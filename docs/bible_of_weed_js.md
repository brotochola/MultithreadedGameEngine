...
- **`contactFriction`** (Float32): contact grip between overlapping colliders (not air drag — that is `RigidBody.friction`). Pair μ = `min(μi, μj)` (not Box2D √). Either side `0` → no grip. Physics applies it by rewriting Verlet `px`/`py` (tangent kill); moving `x` for friction invents velocity.
