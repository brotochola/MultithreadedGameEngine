...
| **RigidBody**           | `active`, `static`, `collisionCount`, `sleeping` (Uint8); `vx`, `vy`, `ax`, `ay`, `px`, `py`, `angularVelocity`, `angularAccel`, `mass`, `invMass`, `inertia`, `invInertia`, `drag`, `angularDrag`, `maxVel`, `minSpeed`, `friction`, `velocityAngle`, `speed`, `stillnessTime` (Float32) |
| **Collider**            | `active`, `shapeType` (0=Circle, 1=Box/AABB, 2=OrientedBox), `isTrigger` (Uint8); `offsetX`, `offsetY`, `radius`, `width`, `height`, `visualRange`, `contactFriction` (Float32; contact grip, pair μ = min; vs RigidBody.friction air drag); `collisionLayer` (Uint8, index 0-31); `collisionMask` (Uint32, bitmask -- 32 collision layers max); `collisionGroupIndex` (Int32, Box2D-style group: 0 = layer/mask only, same negative = never collide, same positive = always collide) |
